...
class MsLowongan extends CI_Model
{
        ...
        {
                $query = $this->db->query("SELECT a.IDLowongan,a.IDPerusahaan,a.IDPosisiJabatan,a.IDKeahlian,a.IDStatusPendidikan,a.IDJenisPengupahan,a.IDStatusHubunganKerja,a.TglBerlaku,a.TglBerakhir,a.NamaLowongan,a.UraianPekerjaan,a.UraianTugas,a.Penempatan,a.BatasUmur,a.JmlPria,a.JmlWanita,a.Jurusan,a.Pengalaman,a.SyaratKhusus,a.GajiPerbulan,a.JamKerjaSeminggu,a.RegisterDate,b.NamaPerusahaan,b.Alamat,b.KodePos,b.EmailPemberiKerja,c.NamaPosisiJabatan,e.NamaStatusPendidikan FROM ".strtolower("MsLowongan")." AS a INNER JOIN ".strtolower("MsPerusahaan")." AS b ON a.IDPerusahaan=b.IDPerusahaan INNER JOIN ".strtolower("MsPosisiJabatan")." AS c ON a.IDPosisiJabatan=c.IDPosisiJabatan INNER JOIN ".strtolower("MsKeahlian")." AS d ON a.IDKeahlian=d.IDKeahlian INNER JOIN ".strtolower("MsStatusPendidikan")." AS e ON a.IDStatusPendidikan=e.IDStatusPendidikan INNER JOIN ".strtolower("MsJenisPengupahan")." AS f ON a.IDJenisPengupahan=f.IDJenisPengupahan INNER JOIN ".strtolower("MsStatusHubunganKerja")." AS g ON a.IDStatusHubunganKerja=g.IDStatusHubunganKerja LIMIT ".$num.($offset != "" ? " OFFSET ".$offset : ""));
                return $query;
        }
