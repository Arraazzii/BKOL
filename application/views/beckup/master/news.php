...
if ($CountMsBeritaData > 0)
{
        ...
        foreach ($MsBeritaData as $getdata)
        {
                ...
                echo '<li>
                        <font size="1">
                                <b>Berita Utama : </b><i>'.$hari[(int)date("w", $gettgl)].', '.(int)date("d", $gettgl).' '.$bulan[(int)date("m", $gettgl)].' '.(int)date("Y", $gettgl).'</i><br /><a href="'.site_url('berita/view').'/'.$getdata->IDBerita.'">'.$getdata->JudulBerita.'</a></font>';
                echo '</li>';
        }
        echo '</ul>';
